...
data_graph3 <- alcool_religion %>%
  ...
  mutate (courant_maj_islam = ifelse(maj_islam == islmsunpct,"Sunnites", ifelse(maj_islam == islmshipct, "Shiites", "Autres"))) %>% 
...
christ <-ggplot(data_graph3, aes(x = chrstgenpct, y = Conso_totale)) + 
  geom_point(aes(color = courant_maj_chrst), alpha = 0.5) +
  geom_smooth(method=lm, se = FALSE)+
  scale_color_manual(values = c("#E7B800","#00AFBB", "#004fff", "#0066ff"), name = "Courant majoritaire")+
  theme_minimal()+
  xlab("Proportion de musulmans dans le pays")+
  ylab("")+
  stat_regline_equation(label.y = 570, label.x = 10, aes(label = ..rr.label..))+
  ggtitle("Consommation d'alcool dans le monde vu au niveau des 2 religions majoritaires ")
christ

islam <-ggplot(data_graph3, aes(x = islmgenpct, y = Conso_totale)) + 
  geom_point(aes(color = courant_maj_islam), alpha = 0.5) +
  geom_smooth(method=lm, se = FALSE, color = "#4d7f17")+
  scale_color_manual(values = c("#4d7f17","#6bb120", "#8ae429"), name = "Courant majoritaire")+
  theme_minimal()+
  xlab("Proportion de musulmans dans le pays")+
  ylab("")+
  stat_regline_equation(label.y = 550, label.x = 60, aes(label = ..rr.label..))+
