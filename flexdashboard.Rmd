...
```{r}
religion_main <- alcool_religion %>% 
    select(Country, ends_with("PerCapita"), -GDP_PerCapita, chrstprotpct, chrstcatpct, chrstorthpct, chrstangpct, chrstothrpct, budgenpct, islmgenpct, judgenpct, nonreligpct) %>%
...
p <- ggplot(data = religion_main) 
p <- p + aes(x=religion, y=value, fill = Type) 
p <- p + geom_boxplot(outlier.shape=NA) 
p <- p +  scale_fill_manual(values=c("darkgoldenrod1","aquamarine4", "darkred"))
p <- p + theme(plot.margin = unit(c(1,1,1,1), "lines"))
p <- p + theme(axis.text.x = element_text(angle = 65, hjust = 1),axis.line = element_line(colour = "black", size = 1, linetype = "solid"))
p <- p + theme(legend.position="bottom") 
p <- p + labs(x="",y="",title="Représentation de la consommation d'alcool par habitant \nen fonction de la religion majoritaire",subtitle="Spécification sur la consommation d'alcool : bière, spiritueux ou vin",fill = "Type d'alcool")
p <- p + theme(plot.title = element_text(face = "bold", size = 16,hjust=0.5), plot.subtitle = element_text(size = 11,hjust=0.5))
p <- p + scale_x_discrete(limits=c("Chrétiens - autres","Chrétiens - catholiques", "Chrétiens - orthodoxes","Chrétiens - protestants", "Athées", "Bouddhistes", "Musulmans"))
p <- p + scale_y_continuous(labels=c("0 L par habitant","100 L par habitant","200 L par habitant","300 L par habitant","400 L par habitant"))
p <- p + annotate("rect", xmin = c(0.5,4.6,5.6,6.5), xmax = c(4.5,5.45,6.45,7.45), ymin = -5, ymax = 410, fill = c("blue","#cc0000","#ffd966","#4d7f17"), alpha = 0.1) 
p <- p + theme(panel.background = element_rect(fill = "white",size = 2))
p
