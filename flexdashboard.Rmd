...
religion_map <- religion_main %>%
  ...
  scale_fill_manual(values = palette_religion, name = '') +
  xlab('') + ylab('') +
  ...
  ggtitle("Religions majoritaires en 2010 dans le monde")
...
alcool_map <- alcool_brut %>% 
  ...
  mutate(Indicator.Name = fct_recode(Indicator.Name,
                                     'alcool_consum' = "Total alcohol consumption per capita (liters of pure alcohol, projected estimates, 15+ years of age)")) %>% 
  pivot_wider(names_from = Indicator.Name, values_from = X2010) %>% 
  rename(iso3c = Country.Code) %>% 
  right_join(countrycode::codelist, by = "iso3c") %>% 
  select(country.name.en, alcool_consum) %>% 
  rename(region = country.name.en) %>% 
  mutate(region = as.factor(region)) %>% 
  mutate(region = fct_recode(region,
                             "USA" = "United States",
...
habitant de plus de 15 ans.",
  ...
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks = element_blank()) +
  ggtitle("Consommation d'alcool en 2010 dans le monde") 

alcool_legend <- alcool_map

alcool_map <- alcool_map + theme(legend.position = 'None')
